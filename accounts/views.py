...
from .serializers import UserSimpleSerializer, UserRequestSerializer, SimilarResponseSerializer
from movies.models import MovieRank
from movies.serializers.swagger import MessageSerializer
...
@swagger_auto_schema(
    ...
    responses={201: openapi.Response("회원가입 완료 후 username을 반환합니다.", UserSimpleSerializer)},
)
@api_view(["POST"])
def signup(request):
...
@swagger_auto_schema(
    ...
    manual_parameters=[token_param],
    responses={200: SimilarResponseSerializer},
)
@api_view(["GET"])
def similar(request):
